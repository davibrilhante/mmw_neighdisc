	...
	for erro in [0,4]:
		...
		for h in dev:	
			...
			for j in range(1,runs):
				filename = dir2+"/"+str(nNodes[2])+sep+str(nBeams)+sep+str(0)+sep+str(erro)+sep+str(h)+sep+str(j)
				f = open(filename)
				line = f.readline()
		...
		if erro == 0:
			plt.errorbar(dev,plot, yerr=bar, c='red', fmt='o')
			ax.plot(dev,plot, 'red', ls='--')
		else:
			plt.errorbar(dev,plot, yerr=bar, c='blue', fmt='o')
			ax.plot(dev,plot, 'blue', ls='--')
