...
if __name__=="__main__":
	...
	for erro in [0,4]:
		...
		if erro == 0:		
			...
			plt.errorbar(nBeams,plot, yerr=bar, c='blue', fmt='o')
			ax.plot(nBeams,plot, 'blue')
	...
	handles, labels = ax.get_legend_handles_labels()
        ...
        ax.legend([leg2, leg4, leg1, leg3],#+[handle for i,handle in enumerate(handles) if i in display],
         ['Sem folga e sem erros', 'Com folga e $\sigma$=20', 'Sem folga e sem erros', 'Sem folga e $\sigma$ = 20'],loc=0, ncol=2) 
	#+[label for i,label in enumerate(labels) if i in display], loc=0, ncol=3)
        plt.show()
