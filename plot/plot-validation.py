...
if __name__=="__main__":
	dir1 = '/home/davi/github/08-12-17' #/mmw/Julho/18-1/sector-norelief-01-22-07-17-07-17 #sys.argv[1]
	dir2 = '/home/dbrilhante/Documents/Codes/mmw/Julho/awgn-norelief-01-36-33-20-07-17'#sys.argv[2]
	#nBeams = sys.argv[1]
	runs = 50
	nNodes = [5, 10, 15, 20, 25] #[10, 20, 30, 40]
	SLRC = [4, 8, 16, 32]
	...
	for slrc in SLRC:
		...
		for n in nNodes:	
			...
			j=1
			while counter <> 29:
				filename = dir1+"/"+str(n)+"-7-"+str(slrc)+sep+str(j)
				f = open(filename)
				check = f.readline()	
				check = float(check)
				j+=1
		...
		if slrc == 8:
			...
			ax.plot(nNodes,plot, 'red', ls='--',label="slrc=8")
		elif slrc == 4:
			plt.errorbar(nNodes,plot, yerr=bar, c='red', fmt='o')
			ax.plot(nNodes,plot, 'blue', ls='--',label="slrc=4")
		elif slrc == 16:
			plt.errorbar(nNodes,plot, yerr=bar, c='red', fmt='o')
			ax.plot(nNodes,plot, 'yellow', ls='--',label="slr=16")
		elif slrc == 32:
			plt.errorbar(nNodes,plot, yerr=bar, c='red', fmt='o')
			ax.plot(nNodes,plot, 'green', ls='--',label="slrc=32")
		'''else:
		...
		print s, tx_time
		if slrc == 8:
			plt.errorbar(nNodes,tx_time, yerr=bar, c='red', fmt='o')
			ax.plot(nNodes,tx_time, 'red',label="slrc=8")
		elif slrc == 4:
			plt.errorbar(nNodes,tx_time, yerr=bar, c='red', fmt='o')
			ax.plot(nNodes,tx_time, 'blue',label="slrc=4")
		elif slrc == 16:
			plt.errorbar(nNodes,tx_time, yerr=bar, c='red', fmt='o')
			ax.plot(nNodes,tx_time, 'yellow',label="slr=16")
		elif slrc == 32:
			plt.errorbar(nNodes,tx_time, yerr=bar, c='red', fmt='o')
			ax.plot(nNodes,tx_time, 'green',label="slrc=32")
		'''
