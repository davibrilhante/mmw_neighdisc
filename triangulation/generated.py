...
if __name__ == "__main__":
	...
	ctrl_mundi = time_arr['txNum']+(nNodes-1)*(time_arr['sifs']+time_arr['txMap'])
        schedule = scheduler.genTraffic(nodes,nNodes, rate, 200) #scheduler.txSched(nodes,nNodes,rate)
	#print schedule
	...
	blockIntervals = []
		...
		for j in range(int(i[2])):
			#print i[0], nodes[i[0]].relays, i[1], nodes[i[1]].relays
			...
			if timeb <> 0:
				blockIntervals.append([i[3],i[3]+timeb/1e6])
